...
if [ "$#" -ne "$num_expected" ]; then
  echo "Usage: ./PlotDetVarCovarianceComparison.sh FPM_Config SYST_Config SLICE_Config Univ_File PlotOutputDir"
  exit 1
fi
...
DetVarCovarianceComparison ${FPM_Config} ${SYST_Config} ${SLICE_Config} ${Univ_File} ${PlotOutputDir}
